...
function upload() {
   ...
   if(!in_array($ekstensiGambar, $ekstensiGambarValid)) { //function in_array untuk mencari apakah ekstensi yang diupload sesuai dengan peraturan ekstensi yang sudah ditetapkan
      echo "
      <script>
...
//function untuk menangani data register
function register($data) {
   ...
   $konfirmasiPassword = mysqli_real_escape_string($db, $data["password2"]);

   //cek apakah username yang diinputkan sudah ada atau belum
   $queryUserNameCheck = "SELECT username FROM user WHERE username = '$username'";
   $result = mysqli_query($db, $queryUserNameCheck);
   ...
   //cek apakah konfirmasi password
   ...
   //enkripsi password sebelum dimasukkan dalam database
